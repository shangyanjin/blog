...
// Service struct - Represents the service for managing page operations
...
var Page = NewPageService()

// Public function - All retrieves all records
func (this *PageService) All(c *gin.Context) *model.Data {
    var rows []model.PageResp
    err := this.DB.Model(&model.Page{}).Order("id desc").Find(&rows).Error
    if err != nil {
        logrus.Errorf("Failed to retrieve all Page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to retrieve all Page"),
        }
    }
...
func (this *PageService) Count(c *gin.Context) *model.Data {
    var count int64
    err := this.DB.Model(&model.Page{}).Count(&count).Error
    if err != nil {
        logrus.Errorf("Failed to count page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to count Page"),
        }
    }
...
func (this *PageService) List(c *gin.Context, pageReq model.PageReq, listReq model.PageListReq) *model.Data {
    if pageReq.Size < 1 {
        pageReq.Size = 10
    }
    if pageReq.Page < 1 {
    ...
    query := this.DB.Model(&model.Page{})
    ...
    if listReq.MetaKeywords != "" {
        query = query.Where("meta_keywords LIKE ?", "%"+listReq.MetaKeywords+"%")
    }
    ...
    if err := query.Limit(limit).Offset(offset).Order("id desc").Find(&rows).Error; err != nil {
        logrus.Errorf("Failed to list Page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to list Page"),
        }
    }
...
func (this *PageService) Detail(c *gin.Context, id int) *model.Data {
    if id < 1 {
        return &model.Data{
    ...
    var row model.PageResp
    err := this.DB.Model(&model.Page{}).Where("id = ?", id).First(&row).Error
    if err != nil {
        logrus.Errorf("Failed to get detail: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to get Page detail"),
        }
    }
...
func (this *PageService) UserAll(c *gin.Context, userId int) *model.Data {
...
func (this *PageService) UserCount(c *gin.Context, userId int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) UserList(c *gin.Context, userId int, pageReq model.PageReq, listReq model.PageListReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) UserDetail(c *gin.Context, userId int, id int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Page{}).Where("id = ?", id)
...
func (this *PageService) UserAdd(c *gin.Context, userId int, addReq model.PageAddReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    err :=  this.DB.Model(&model.Page{}).Create(&addReq).Error
    if err != nil {
        logrus.Errorf("Failed to add Page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to add Page"),
        }
    }
...
func (this *PageService) UserEdit(c *gin.Context, userId int, editReq model.PageEditReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Page{}).Where("id = ?", editReq.Id)
    ...
    if editReq.MetaKeywords != "" {
        updateData["meta_keywords"] = editReq.MetaKeywords
    }
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to edit Page with Id %d: %v", editReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to update Page"),
        }
    }
...
func (this *PageService) UserChange(c *gin.Context, userId int, changeReq model.PageChangeReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Page{}).Where("id = ?", changeReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to change Page with Id %d: %v", changeReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to change Page"),
        }
    }
...
func (this *PageService) UserDel(c *gin.Context, userId int, delReq model.PageIdReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Page{}).Error; err != nil {
        logrus.Errorf("Failed to delete Page with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to delete Page"),
        }
    }
...
func (this *PageService) UserDels(c *gin.Context, userId int, delsReq model.PageIdsReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Page{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to batch delete Page"),
        }
    }
...
func (this *PageService) AdminAll(c *gin.Context, adminId int) *model.Data {
...
func (this *PageService) AdminCount(c *gin.Context, adminId int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) AdminList(c *gin.Context, adminId int, pageReq model.PageReq, listReq model.PageListReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) AdminDetail(c *gin.Context, adminId int, id int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) AdminAdd(c *gin.Context, adminId int, addReq model.PageAddReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) AdminEdit(c *gin.Context, adminId int, editReq model.PageEditReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *PageService) AdminDel(c *gin.Context, delReq model.PageIdReq) *model.Data {
    if delReq.Id < 1 {
        return &model.Data{
            Code:    http.StatusBadRequest,
            Error:   errors.New("Invalid Id"),
    ...
    err := this.DB.Where("id = ?", delReq.Id).Delete(&model.Page{}).Error
    if err != nil {
        logrus.Errorf("Failed to delete Page with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
...
func (this *PageService) AdminDels(c *gin.Context, adminId int, delsReq model.PageIdsReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
    ...
    if err := this.DB.Where("id IN ?", idList).Delete(&model.Page{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Page: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
...
func (this *PageService) AdminChange(c *gin.Context, adminId int, changeReq model.PageChangeReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
