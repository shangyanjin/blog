...
// Service struct - Represents the service for managing tag operations
...
func (this *TagService) All(c *gin.Context) *model.Data {
    var rows []model.TagResp
    err := this.DB.Model(&model.Tag{}).Order("id desc").Find(&rows).Error
    if err != nil {
        logrus.Errorf("Failed to retrieve all Tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to retrieve all Tag"),
        }
    }
...
func (this *TagService) Count(c *gin.Context) *model.Data {
    var count int64
    err := this.DB.Model(&model.Tag{}).Count(&count).Error
    if err != nil {
        logrus.Errorf("Failed to count tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to count Tag"),
        }
    }
...
func (this *TagService) List(c *gin.Context, pageReq model.PageReq, listReq model.TagListReq) *model.Data {
    if pageReq.Size < 1 {
        pageReq.Size = 10
    }
    if pageReq.Page < 1 {
    ...
    if err := query.Limit(limit).Offset(offset).Order("id desc").Find(&rows).Error; err != nil {
        logrus.Errorf("Failed to list Tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to list Tag"),
        }
    }
...
func (this *TagService) Detail(c *gin.Context, id int) *model.Data {
    if id < 1 {
        return &model.Data{
    ...
    var row model.TagResp
    err := this.DB.Model(&model.Tag{}).Where("id = ?", id).First(&row).Error
    if err != nil {
        logrus.Errorf("Failed to get detail: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to get Tag detail"),
        }
    }
...
func (this *TagService) UserAll(c *gin.Context, userId int) *model.Data {
...
func (this *TagService) UserCount(c *gin.Context, userId int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) UserList(c *gin.Context, userId int, pageReq model.PageReq, listReq model.TagListReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) UserDetail(c *gin.Context, userId int, id int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Tag{}).Where("id = ?", id)
...
func (this *TagService) UserAdd(c *gin.Context, userId int, addReq model.TagAddReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    err :=  this.DB.Model(&model.Tag{}).Create(&addReq).Error
    if err != nil {
        logrus.Errorf("Failed to add Tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to add Tag"),
        }
    }
...
func (this *TagService) UserEdit(c *gin.Context, userId int, editReq model.TagEditReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Tag{}).Where("id = ?", editReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to edit Tag with Id %d: %v", editReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to update Tag"),
        }
    }
...
func (this *TagService) UserChange(c *gin.Context, userId int, changeReq model.TagChangeReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Tag{}).Where("id = ?", changeReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to change Tag with Id %d: %v", changeReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to change Tag"),
        }
    }
...
func (this *TagService) UserDel(c *gin.Context, userId int, delReq model.TagIdReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Tag{}).Error; err != nil {
        logrus.Errorf("Failed to delete Tag with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to delete Tag"),
        }
    }
...
func (this *TagService) UserDels(c *gin.Context, userId int, delsReq model.TagIdsReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Tag{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to batch delete Tag"),
        }
    }
...
func (this *TagService) AdminAll(c *gin.Context, adminId int) *model.Data {
...
func (this *TagService) AdminCount(c *gin.Context, adminId int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) AdminList(c *gin.Context, adminId int, pageReq model.PageReq, listReq model.TagListReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) AdminDetail(c *gin.Context, adminId int, id int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) AdminAdd(c *gin.Context, adminId int, addReq model.TagAddReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) AdminEdit(c *gin.Context, adminId int, editReq model.TagEditReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *TagService) AdminDel(c *gin.Context, delReq model.TagIdReq) *model.Data {
    if delReq.Id < 1 {
        return &model.Data{
            Code:    http.StatusBadRequest,
            Error:   errors.New("Invalid Id"),
    ...
    err := this.DB.Where("id = ?", delReq.Id).Delete(&model.Tag{}).Error
    if err != nil {
        logrus.Errorf("Failed to delete Tag with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
...
func (this *TagService) AdminDels(c *gin.Context, adminId int, delsReq model.TagIdsReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
    ...
    if err := this.DB.Where("id IN ?", idList).Delete(&model.Tag{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Tag: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
...
func (this *TagService) AdminChange(c *gin.Context, adminId int, changeReq model.TagChangeReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
