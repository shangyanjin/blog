...
// Service struct - Represents the service for managing verify operations
...
var Verify = NewVerifyService()

// Public function - All retrieves all records
func (this *VerifyService) All(c *gin.Context) *model.Data {
    var rows []model.VerifyResp
    err := this.DB.Model(&model.Verify{}).Order("id desc").Find(&rows).Error
    if err != nil {
        logrus.Errorf("Failed to retrieve all Verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to retrieve all Verify"),
        }
    }
...
func (this *VerifyService) Count(c *gin.Context) *model.Data {
    var count int64
    err := this.DB.Model(&model.Verify{}).Count(&count).Error
    if err != nil {
        logrus.Errorf("Failed to count verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to count Verify"),
        }
    }
...
func (this *VerifyService) List(c *gin.Context, pageReq model.PageReq, listReq model.VerifyListReq) *model.Data {
    if pageReq.Size < 1 {
        pageReq.Size = 10
    }
    if pageReq.Page < 1 {
    ...
    query := this.DB.Model(&model.Verify{})
    ...
    // }
    // if !listReq.ExpiredAt.IsZero() {
    //     query = query.Where("expired_at = ?", listReq.ExpiredAt)
    // }
    ...
    if err := query.Limit(limit).Offset(offset).Order("id desc").Find(&rows).Error; err != nil {
        logrus.Errorf("Failed to list Verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to list Verify"),
        }
    }
...
func (this *VerifyService) Detail(c *gin.Context, id int) *model.Data {
    if id < 1 {
        return &model.Data{
    ...
    var row model.VerifyResp
    err := this.DB.Model(&model.Verify{}).Where("id = ?", id).First(&row).Error
    if err != nil {
        logrus.Errorf("Failed to get detail: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to get Verify detail"),
        }
    }
...
func (this *VerifyService) UserAll(c *gin.Context, userId int) *model.Data {
...
func (this *VerifyService) UserCount(c *gin.Context, userId int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) UserList(c *gin.Context, userId int, pageReq model.PageReq, listReq model.VerifyListReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) UserDetail(c *gin.Context, userId int, id int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Verify{}).Where("id = ?", id)
...
func (this *VerifyService) UserAdd(c *gin.Context, userId int, addReq model.VerifyAddReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    err :=  this.DB.Model(&model.Verify{}).Create(&addReq).Error
    if err != nil {
        logrus.Errorf("Failed to add Verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to add Verify"),
        }
    }
...
func (this *VerifyService) UserEdit(c *gin.Context, userId int, editReq model.VerifyEditReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Verify{}).Where("id = ?", editReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to edit Verify with Id %d: %v", editReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to update Verify"),
        }
    }
...
func (this *VerifyService) UserChange(c *gin.Context, userId int, changeReq model.VerifyChangeReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Verify{}).Where("id = ?", changeReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to change Verify with Id %d: %v", changeReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to change Verify"),
        }
    }
...
func (this *VerifyService) UserDel(c *gin.Context, userId int, delReq model.VerifyIdReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Verify{}).Error; err != nil {
        logrus.Errorf("Failed to delete Verify with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to delete Verify"),
        }
    }
...
func (this *VerifyService) UserDels(c *gin.Context, userId int, delsReq model.VerifyIdsReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Verify{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to batch delete Verify"),
        }
    }
...
func (this *VerifyService) AdminAll(c *gin.Context, adminId int) *model.Data {
...
func (this *VerifyService) AdminCount(c *gin.Context, adminId int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) AdminList(c *gin.Context, adminId int, pageReq model.PageReq, listReq model.VerifyListReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) AdminDetail(c *gin.Context, adminId int, id int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) AdminAdd(c *gin.Context, adminId int, addReq model.VerifyAddReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) AdminEdit(c *gin.Context, adminId int, editReq model.VerifyEditReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *VerifyService) AdminDel(c *gin.Context, delReq model.VerifyIdReq) *model.Data {
    if delReq.Id < 1 {
        return &model.Data{
            Code:    http.StatusBadRequest,
            Error:   errors.New("Invalid Id"),
    ...
    err := this.DB.Where("id = ?", delReq.Id).Delete(&model.Verify{}).Error
    if err != nil {
        logrus.Errorf("Failed to delete Verify with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
...
func (this *VerifyService) AdminDels(c *gin.Context, adminId int, delsReq model.VerifyIdsReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
    ...
    if err := this.DB.Where("id IN ?", idList).Delete(&model.Verify{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Verify: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
...
func (this *VerifyService) AdminChange(c *gin.Context, adminId int, changeReq model.VerifyChangeReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
