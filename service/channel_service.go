...
// Service struct - Represents the service for managing channel operations
...
var Channel = NewChannelService()

// Public function - All retrieves all records
func (this *ChannelService) All(c *gin.Context) *model.Data {
    var rows []model.ChannelResp
    err := this.DB.Model(&model.Channel{}).Order("id desc").Find(&rows).Error
    if err != nil {
        logrus.Errorf("Failed to retrieve all Channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to retrieve all Channel"),
        }
    }
...
func (this *ChannelService) Count(c *gin.Context) *model.Data {
    var count int64
    err := this.DB.Model(&model.Channel{}).Count(&count).Error
    if err != nil {
        logrus.Errorf("Failed to count channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to count Channel"),
        }
    }
...
func (this *ChannelService) List(c *gin.Context, pageReq model.PageReq, listReq model.ChannelListReq) *model.Data {
    if pageReq.Size < 1 {
        pageReq.Size = 10
    }
    if pageReq.Page < 1 {
    ...
    query := this.DB.Model(&model.Channel{})
    ...
    if listReq.CreatorId > 0 {
        query = query.Where("creator_id = ?", listReq.CreatorId)
    }
    ...
    if err := query.Limit(limit).Offset(offset).Order("id desc").Find(&rows).Error; err != nil {
        logrus.Errorf("Failed to list Channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to list Channel"),
        }
    }
...
func (this *ChannelService) Detail(c *gin.Context, id int) *model.Data {
    if id < 1 {
        return &model.Data{
    ...
    var row model.ChannelResp
    err := this.DB.Model(&model.Channel{}).Where("id = ?", id).First(&row).Error
    if err != nil {
        logrus.Errorf("Failed to get detail: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to get Channel detail"),
        }
    }
...
func (this *ChannelService) UserAll(c *gin.Context, userId int) *model.Data {
...
func (this *ChannelService) UserCount(c *gin.Context, userId int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) UserList(c *gin.Context, userId int, pageReq model.PageReq, listReq model.ChannelListReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) UserDetail(c *gin.Context, userId int, id int) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Channel{}).Where("id = ?", id)
...
func (this *ChannelService) UserAdd(c *gin.Context, userId int, addReq model.ChannelAddReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    err :=  this.DB.Model(&model.Channel{}).Create(&addReq).Error
    if err != nil {
        logrus.Errorf("Failed to add Channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to add Channel"),
        }
    }
...
func (this *ChannelService) UserEdit(c *gin.Context, userId int, editReq model.ChannelEditReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Channel{}).Where("id = ?", editReq.Id)
    ...
    // Skip sensitive fields
    if editReq.CreatorId > 0 {
        updateData["creator_id"] = editReq.CreatorId
    }
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to edit Channel with Id %d: %v", editReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to update Channel"),
        }
    }
...
func (this *ChannelService) UserChange(c *gin.Context, userId int, changeReq model.ChannelChangeReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    query := this.DB.Model(&model.Channel{}).Where("id = ?", changeReq.Id)
    ...
    if err := query.Updates(updateData).Error; err != nil {
        logrus.Errorf("Failed to change Channel with Id %d: %v", changeReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to change Channel"),
        }
    }
...
func (this *ChannelService) UserDel(c *gin.Context, userId int, delReq model.ChannelIdReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Channel{}).Error; err != nil {
        logrus.Errorf("Failed to delete Channel with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to delete Channel"),
        }
    }
...
func (this *ChannelService) UserDels(c *gin.Context, userId int, delsReq model.ChannelIdsReq) *model.Data {
    userInfo, userErr := model.GetUserInfo(c)
    if userErr != nil {
        logrus.Errorf("Failed to get user %d: %v, info: %+v", userId, userErr, userInfo)
        return &model.Data{Code: http.StatusForbidden, Error: userErr, Message: "Error 403: Forbidden"}
    ...
    if err := query.Delete(&model.Channel{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
            Message: fmt.Sprintf("Error 500: Failed to batch delete Channel"),
        }
    }
...
func (this *ChannelService) AdminAll(c *gin.Context, adminId int) *model.Data {
...
func (this *ChannelService) AdminCount(c *gin.Context, adminId int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) AdminList(c *gin.Context, adminId int, pageReq model.PageReq, listReq model.ChannelListReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) AdminDetail(c *gin.Context, adminId int, id int) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) AdminAdd(c *gin.Context, adminId int, addReq model.ChannelAddReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) AdminEdit(c *gin.Context, adminId int, editReq model.ChannelEditReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
...
func (this *ChannelService) AdminDel(c *gin.Context, delReq model.ChannelIdReq) *model.Data {
    if delReq.Id < 1 {
        return &model.Data{
            Code:    http.StatusBadRequest,
            Error:   errors.New("Invalid Id"),
    ...
    err := this.DB.Where("id = ?", delReq.Id).Delete(&model.Channel{}).Error
    if err != nil {
        logrus.Errorf("Failed to delete Channel with Id %d: %v", delReq.Id, err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
...
func (this *ChannelService) AdminDels(c *gin.Context, adminId int, delsReq model.ChannelIdsReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
    ...
    if err := this.DB.Where("id IN ?", idList).Delete(&model.Channel{}).Error; err != nil {
        logrus.Errorf("Failed to batch delete Channel: %v", err)
        return &model.Data{
            Code:    http.StatusInternalServerError,
            Error:   err,
...
func (this *ChannelService) AdminChange(c *gin.Context, adminId int, changeReq model.ChannelChangeReq) *model.Data {
    adminInfo, adminErr := model.GetAdminInfo(c)
    if adminErr != nil {
        logrus.Errorf("Failed to get admin %d: %v, info: %+v", adminId, adminErr, adminInfo)
        return &model.Data{Code: http.StatusForbidden, Error: adminErr, Message: "Error 403: Forbidden"}
